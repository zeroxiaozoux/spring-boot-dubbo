...
public class AnnotationBean extends AbstractConfig implements DisposableBean, BeanFactoryPostProcessor,
    ...
    protected void postProcessAnnotationPackageService() {
        ...
        log.debug("{} annotated @Service Components { {} } were scanned under package[{}]", definitionHolders.size(), definitionHolders, this.annotationPackages);
    }
...
//            registerWithGeneratedName(ExportBean.build((Service) export, beanName, interfaces), registry);
//        }
//    }
    ...
    private void registerServiceBean(Service service, Class<?> beanClass, String beanName) {
        ...
        if (interfaceClass == null) {
            ...
            for (Class<?> interfaces : interfacess) {
                registerWithGeneratedName(ServiceBeanUtil.build(service, beanName, interfaces), registry);
            }
        } else {
            registerWithGeneratedName(ServiceBeanUtil.build(service, beanName, interfaceClass), registry);
        }
    }
