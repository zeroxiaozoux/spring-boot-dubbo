...
import com.dubbo.filter.ConsumerFilter;
import com.dubbo.filter.ConsumerFilterBean;
import com.dubbo.filter.ProviderFilter;
import com.dubbo.filter.ProviderFilterBean;
...
public class DubboAutoConfiguration extends AnnotationBean
    ...
    private void registerConsumer(ConsumerConfig consumer, ConfigurableListableBeanFactory beanFactory) {
        if (consumer != null) {
            ...
            log.debug("配置 ConsumerFilter的父节点Filter, {}", filter);
    ...
    private void registerProvider(ProviderConfig provider, ConfigurableListableBeanFactory beanFactory) {
        if (provider != null) {
            ...
            log.debug("配置 ProviderFilter的父节点Filter, {}", filter);
    ...
    private void registerReferences(List<ReferenceConfig<?>> references, ConfigurableListableBeanFactory beanFactory) {
        ...
        for (int index = 0; index < references.size(); index++) {
            ...
            log.debug("注册服务调用信息{} 完毕", beanName);
        }
    }
...
//			logger.info("dubbo没有配置注解服务所在的包路径,如果你使用注解扫描，该日志可忽略。");
            ...
            log.info("dubbo开始扫描: {}", basePackages);
        }
    ...
    public void afterPropertiesSet() throws Exception {
        try {
            ...
            log.debug("没有添加自定义ConsumerFilter");
        }
        try {
            ...
            log.debug("没有添加自定义ProviderFilter");
        }
    }
